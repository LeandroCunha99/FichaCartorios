...
def ocr_pdf(path):
    with tempfile.TemporaryDirectory() as td:
        ...
        return "\n\n".join(ocr_image(p) for p in pages), len(pages)
...
def extract_document(data, filename, mimetype):
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        if suffix == ".pdf" or (mimetype and "pdf" in mimetype.lower()):
            direct = ""
            try: direct = extract_pdf_text(p).strip()
            except Exception: direct = ""
            if len(direct) >= 80:
                return {"ok": True, "mode": "PDF_TEXTUAL", "pages": None, "text": direct}
            text, pages = ocr_pdf(p)
            return {"ok": True, "mode": "PDF_OCR", "pages": pages, "text": text}
        if suffix in {".png",".jpg",".jpeg",".tif",".tiff",".webp",".bmp"} or (mimetype and mimetype.lower().startswith("image/")):
            return {"ok": True, "mode": "IMAGE_OCR", "pages": 1, "text": ocr_image(p)}
        if suffix == ".txt" or (mimetype and "text/plain" in mimetype.lower()):
            return {"ok": True, "mode": "TEXT", "pages": 1, "text": data.decode("utf-8", errors="replace")}
...
def ocr_health():
    ...
    return jsonify({"ok": True, "ocr_ready": ready, "version": "Beta Cartório 1"})
...
def ocr_extract():
    ...
    try:
        return jsonify(extract_document(f.read(), f.filename or "documento", f.mimetype))
...
def system_health():
    return jsonify({"ok": True, "app": "Plataforma Notarial Integrada", "version": "Beta Cartório 1"})
